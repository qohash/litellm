...
def test_qostodian_nexus_initialization_with_all_params():
    """Test QostodianNexus initializes with all custom parameters."""
    from litellm.proxy.guardrails.guardrail_hooks.qohash import QostodianNexus

    guardrail = QostodianNexus(
        api_base="http://test-nexus:8888",
        api_key="test-key-123",
    )

    assert "test-nexus:8888" in guardrail.api_base
...
def test_qostodian_nexus_env_vars():
    """Test that QOSTODIAN_NEXUS_* env vars are picked up correctly."""
    import os
    from unittest.mock import patch
    from litellm.proxy.guardrails.guardrail_hooks.qohash import QostodianNexus
    ...
    with patch.dict(os.environ, {"QOSTODIAN_NEXUS_API_KEY": "new-key"}):
        guardrail = QostodianNexus()
...
def test_qostodian_nexus_config_model_field_descriptions():
    ...
    api_key_field = QostodianNexusConfigModel.model_fields["api_key"]
    assert "QOSTODIAN_NEXUS_API_KEY" in api_key_field.description
...
def test_qostodian_nexus_unified_detection():
    ...
    instance = QostodianNexus(api_base="http://test:8800", api_key="test")
